...
void move(int robot, char move, int theRobots[], char theBoard[ BOARD_SIZE][ NUMBER_OF_ELEMENTS], char destinationLetter)
{
	...
	else if (move == 'R')
	{
		...
		while(	(theBoard[robotIndex][RIGHT] != '|') 
				...
				&& ((robotIndex % 16) < 0) // so it does not go past rightmost position in row
				&& (theBoard[robotIndex + 1][PIECE] == '.' || theBoard[robotIndex + 1][PIECE] == destinationLetter))
		{
		    theBoard[ robotIndex][ PIECE] = '.';         // blank out where we're coming FROM
		    theRobots[ robot] = robotIndex + 1;     // Reset robot position index to the square above
...
int main()
{
    ...
    if (inputLetter == 'R')
    {
    	destinationLetter = rand() % (numberOfDestinationPieces) + 65; // + 65 so it's a capital
    	goalRobot = rand() % 4 + 1; // +! since rand can give zero as a value
	}
	...
	else if (inputLetter == 'S')
	{
		...
		cout << "Input goal robot and destination letter in that order.";
		cin >> goalRobot;
